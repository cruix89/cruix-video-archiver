...
check_ffmpeg() {
    if ! command -v ffmpeg &> /dev/null; then
        echo -e "[cruix-video-archiver-hvec] FFMPEG is Not Installed. The Force is Weak in This System, No Media Manipulation Powers!"
        exit 1
    fi
}
...
load_normalized_list() {
    ...
    echo -e "[cruix-video-archiver-hvec] Number of Normalized Files In Cache: ${#normalized_files[@]} Cache is Grooving!"
}
...
process_file() {
    ...
    # FFMPEG command to normalize audio, re-encode video, and combine
    {
        ...
        echo -e "[cruix-video-archiver-hvec] Merging Video and Audio For: $src_file . Crafting the Perfect Symphony!"
        sleep 15
    ...
    if [[ -f "${output_file}_x265.mp4" && $exit_code_audio -eq 0 && $exit_code_video -eq 0 && $exit_code_combine -eq 0 ]]; then
        ...
        echo -e "[cruix-video-archiver-hvec] Processed and Replaced: ${src_file%.*}.mp4 "
        rm -f "$cache_dir"/*
        echo -e "[cruix-video-archiver-hvec] Cleaning Cache Directory: $cache_dir "
    else
...
main() {
    ...
    if [[ ! -d "$cache_dir" ]]; then
        mkdir -p "$cache_dir"
        echo "[cruix-video-archiver-hvec] Created Cache Directory: $cache_dir "
    fi
