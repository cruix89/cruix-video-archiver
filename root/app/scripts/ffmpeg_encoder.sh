...
load_normalized_list() {
    ...
    echo -e "[cruix-video-archiver-hvec] Number of Normalized Files In Cache: ${#normalized_files[@]} 🗄️  Cache is Grooving! 🕺"
}
...
process_file() {
    ...
    if [[ -f "${output_file}_x265.mp4" && $exit_code_audio -eq 0 && $exit_code_video -eq 0 && $exit_code_combine -eq 0 ]]; then
        ...
        echo -e "[cruix-video-archiver-hvec] Processed and Replaced: ${src_file%.*}.mp4  🎛️"
        rm -f "$cache_dir"/*
        echo -e "[cruix-video-archiver-hvec] Cleaning Cache Directory: $cache_dir  🗑️"
    else
